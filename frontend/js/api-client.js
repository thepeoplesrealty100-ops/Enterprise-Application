 ...
 *   const fleet = await client.getDeviceFleet({ client: 'acme', page: 1 });
...
class JAKALClient {
   ...
   */
  async getSettingsTab(tab) {
    const cacheKey = `settings-${tab}`;
    const cached = this._getCache(cacheKey);
    if (cached) return cached;

    const result = await this._request('GET', `/dashboard/settings/${tab}`);
    this._setCache(cacheKey, result);
    return result;
  }
   ...
   * Update a setting
   */
  async updateSetting(settingKey, settingValue, operatorId = 'system') {
    const payload = {
    ...
    return this._request('POST', `/dashboard/settings/general`, payload);
  }
  ...
  async getFabricStatus() {
    ...
    const result = await this._request('GET', `/fabric/status`);
    this._setCache(cacheKey, result);
    return result;
  }
  ...
  async getScriptCatalog(filters = {}) {
    ...
    const result = await this._request('GET', `/scripts/catalog?${query}`);
    this._setCache(cacheKey, result);
    return result;
  }
  ...
  async getResonancePolicies() {
    ...
    const result = await this._request('GET', `/resonance/policies`);
    this._setCache(cacheKey, result);
    return result;
  }
  ...
  async createResonancePolicy(policyData) {
    this.clearCache(); // Invalidate cache
    return this._request('POST', `/resonance/policies`, policyData);
  }
  ...
  async getResonanceAudit(filters = {}) {
    ...
    return this._request('GET', `/resonance/audit?${query}`);
  }
  ...
  async getMetrics() {
    try {
      const response = await fetch(`${this.apiBase}/telemetry/metrics`);
  ...
  connectTelemetry(onEvent, onError = null) {
    ...
    try {
      ...
      console.error('Error connecting to telemetry stream:', error);
      if (onError) onError(error);
    }
  }
